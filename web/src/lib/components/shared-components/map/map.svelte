<script lang="ts">
  ...
  $: {
    let url = `${AppRoute.PHOTOS}?${QueryParameter.COORDINATESX1}=${x1}&${QueryParameter.COORDINATESX2}=${x2}&${QueryParameter.COORDINATESY1}=${y1}&${QueryParameter.COORDINATESY2}=${y2}&${QueryParameter.PREVIOUS_ROUTE}=${btoa(`${AppRoute.MAP}?${QueryParameter.IS_TIMELINE_OPENED}=${isAssetGridOpened}${location.hash}`)}`;
    let options: string[] = [];
  ...
  const changeBounds = () => {
    if (!$mapSettings.includeTimeline) {
      return;
    }
    if (map) {
...
{#await style then style}
  ...
  >
    ...
    {#if showAssetGrid && !$mapSettings.withSharedAlbums && !$mapSettings.includeArchived && $mapSettings.includeTimeline}
      <div
